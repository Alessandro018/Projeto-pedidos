...
db-conf:
	mysql -u root -p --execute="drop database if exists projeto_pedidos; create database projeto_pedidos; drop user if exists 'laravel'; create user 'laravel' identified by 'laravel'; grant all privileges on projeto_pedidos.* to 'laravel';"
	sed -i 's/DB_DATABASE.*/DB_DATABASE=projeto_pedidos/' .env
